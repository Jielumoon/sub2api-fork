...
const colors = computed(() => ({
  ...
  primaryAlpha: `${chartPalette[0]}20`,
  grid: isDarkMode.value ? '#3f3d3a' : '#f4f2ed',
  text: isDarkMode.value ? '#9a9893' : '#76726d'
}))
...
const options = computed(() => {
  ...
  return {
    ...
    plugins: {
      ...
      tooltip: {
        backgroundColor: isDarkMode.value ? '#2c2a28' : '#ffffff',
        titleColor: isDarkMode.value ? '#f2f0ec' : '#201e1c',
        bodyColor: isDarkMode.value ? '#c0bdb8' : '#5d5a56',
        borderColor: c.grid,
        borderWidth: 1,
        padding: 10,
        displayColors: true,
